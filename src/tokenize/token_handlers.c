...
void	handle_token_expansion(t_word_type previous_word_type, t_command_part **command_node, t_env *env)
{
	...
	{
		expanded = expand_token(*command_node, env);
...
void	handle_token_join(t_tokenizer_ctx *ctx)
{
	...
	{
		joined_node = create_command_node(ctx->partial_token, ctx->previous_word_type);
		add_command_part_to_list(&ctx->lst, joined_node);
		free(ctx->partial_token);
		ctx->partial_token = NULL;
	}
	else if ((ctx->previous_word_type == W_REDAP || ctx->previous_word_type == W_HRDOC || ctx->previous_word_type == W_REDIN || ctx->previous_word_type == W_REDOU || ctx->word_type == W___END) && !ctx->partial_token)
	{
