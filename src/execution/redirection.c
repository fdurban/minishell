...
/*   Updated: 2025/07/04 13:01:05 by igngonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	apply_redirections(t_pipex *px)
{
	...
	heredoc_filename = px->heredoc_filenames[px->idx];
	if (heredoc_filename)
	{
		px->in_fd = open(heredoc_filename, O_RDONLY);
		if (px->in_fd < 0)
			handle_error("open input heredoc");
	}
	else if (px->in_fd < 0)
